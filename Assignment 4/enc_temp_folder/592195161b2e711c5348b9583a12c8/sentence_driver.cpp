...
public:
	...
	friend ostream& operator<<(ostream& out, sentence& lineS);
...
ostream& operator<<(ostream& out, sentence& lineS) {
	word* frontHolder = new word;
	frontHolder = lineS.front;
	for (int i = 0; i < lineS.num_of_terms; i++) {
		out << lineS.front->term;
		lineS.front = lineS.front->next;
	}
	lineS.front = frontHolder;
	return out;
}
